 ...
 * @LastEditTime: 2024-11-23 11:49:42
...
RedisConnectionMgr::ErrorCode RedisConnectionMgr::hget(const std::string&key,const std::string&hkey,std::string&ret_val)
{
    std::shared_ptr<RedisConnection> redis = this->_pool.getRedisConnection();
    if(redis == nullptr){
        return RedisConnectionMgr::ErrorCode::STOP;
    ...
    redisContext* redis_connect = redis->_conn;
    redisReply *redis_reply = (redisReply*)redisCommand(redis_connect,"hget %s %s",key.c_str(),hkey.c_str());
    if(redis_reply==nullptr){
        this->_pool.storeRedisConnection(redis);
        return RedisConnectionMgr::ErrorCode::NETWORK_ERR;
    }
    if(redis_reply->type == REDIS_REPLY_ERROR || redis_reply->type != REDIS_REPLY_STRING || redis_reply->type==REDIS_REPLY_NIL){
        freeReplyObject(redis_reply);
        this->_pool.storeRedisConnection(redis);
        return RedisConnectionMgr::ErrorCode::FAILURE;
    }
    ret_val = std::string(redis_reply->str);
    freeReplyObject(redis_reply);
    this->_pool.storeRedisConnection(redis);
    redis->_last_opt_time = std::chrono::steady_clock::now();
    return RedisConnectionMgr::ErrorCode::SUCCESS;
...
RedisConnectionMgr::ErrorCode RedisConnectionMgr::hdel(const std::string&key,const std::string&hkey)
{
    std::shared_ptr<RedisConnection> redis = this->_pool.getRedisConnection();
    if(redis == nullptr){
        return RedisConnectionMgr::ErrorCode::STOP;
    ...
    redisContext* redis_connect = redis->_conn;
    redisReply *redis_reply = (redisReply*)redisCommand(redis_connect,"hdel %s %s",key.c_str(),hkey.c_str());
    if(redis_reply==nullptr){
        this->_pool.storeRedisConnection(redis);
        return RedisConnectionMgr::ErrorCode::NETWORK_ERR;
    }
    if(redis_reply->type == REDIS_REPLY_ERROR || redis_reply->len == 0){
        freeReplyObject(redis_reply);
        this->_pool.storeRedisConnection(redis);
        return RedisConnectionMgr::ErrorCode::FAILURE;
    }
